...
export default function CounselRequestPage() {
    ...
    const handleSubmit = async () => {
        ...
        if (!selectedPastor) {
            Alert.alert('알림', '상담 받을 교역자를 선택해주세요.');
            return;
        }
                ...
                >
                    <View style={{ flex: 1, padding: spacing.lg }}>
                        ...
                        <View style={{ marginBottom: spacing.md }}>
                            ...
                            <Dropdown
                                style={{ height: 50, borderColor: colors.border, borderWidth: 1, borderRadius: radius.sm, paddingHorizontal: 12, backgroundColor: colors.surface }}
                        ...
                        <TextInput
                            ...
                            multiline
                            style={{ borderBottomWidth: 1, borderColor: colors.border, color: colors.text, fontSize: font.body, marginBottom: spacing.lg, paddingVertical: spacing.sm }}
                        />

                        {/* 제출 버튼 */}
                        <TouchableOpacity
                            onPress={handleSubmit}
                            style={{ backgroundColor: colors.primary, paddingVertical: spacing.md, borderRadius: radius.md, alignItems: 'center' }}
                        >
                            <Text style={{ color: '#fff', fontWeight: 'bold', fontSize: font.body }}>💌 상담 요청하기</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <Modal visible={isHistoryModalVisible} animationType="slide">
                        <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
                            <TouchableOpacity onPress={handleCloseHistory} style={{ alignSelf: 'flex-end', padding: 16 }}>
                            ...
                            <ScrollView contentContainerStyle={{ paddingHorizontal: 20 }}>
                                {myCounselHistory.length > 0 ? (
                                    myCounselHistory.map((item, index) => (
                                        <View key={item.id || index} style={{ backgroundColor: colors.surface, padding: 16, borderRadius: 12, marginBottom: 12 }}>
                                            {item.pastorName && (
                                            ...
                                            <Text style={{ color: colors.subtext, fontSize: 12, marginTop: 6 }}>
                                                작성일: {item.createdAt?.seconds ? new Date(item.createdAt.seconds * 1000).toLocaleDateString() : '-'}
                                            </Text>
                                        </View>
                                    ))
                                ) : (
                                    <Text style={{ color: colors.subtext, textAlign: 'center' }}>상담 내역이 없습니다.</Text>
                                )}
                            </ScrollView>
                        </SafeAreaView>
                    </Modal>
